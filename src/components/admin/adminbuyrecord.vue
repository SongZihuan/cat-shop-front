...
import {
  apiAdminPostAcceptFaHuoQuXiaoDengJi, apiAdminPostChangeShop, apiAdminPostChangeUser,
  apiAdminPostFaHuoCheHui, apiAdminPostFaHuoDengJi,
  apiAdminPostFaHuoQuXiao,
  apiAdminPostGouWuDaoHuo,
  apiAdminPostGouWuTuiHuoDaohuo,
  apiAdminPostQuXiaoPay, apiAdminPostTuiHuoAccept, apiAdminPostTuiHuoDengJi, apiAdminPostTuiHuoShenQingAccept
} from "#/admin/huo"
...
const onClassClick = () => {
  record.value && record.value.userId > 1 && pushTo(router, route, "/admin/class/info", {
    classId: record.value.wupin.classId,
  })
}

const onUserClick = () => {
  record.value && record.value.userId && pushTo(router, route, "/admin/user/info", {
  ...
  <el-card v-if="record" class="base_card">
      ...
      <div style="display: flow-root">
        <div style="float: left">
          ...
          <div class="buyer_name_box">
            <el-text class="buyer_name" @click="onUserClick"> 购买人（非账户名）：{{ record.user.name || "未知用户" }} </el-text>
          </div>
        </div>
        <div style="float: right">
          <el-button-group>
            ...
            <el-button v-if="safe && record.wupin && record.wupin.classOf" type="success" size="large" class="class_btn" @click="onClassClick">
              查看商品类型
            </el-button>
            <el-button v-if="safe && record.wupin && record.wupin.classOf" type="success" size="large" class="class_btn" @click="onUserClick">
              查看购物人信息
            </el-button>
          </el-button-group>
        </div>
    ...
    <template #footer>
      <div  class="footer_btn">
        ...
        <el-button v-if="safe && record.status === 4" type="danger" @click="cheHuiFahuo">
          撤回发货
        </el-button>
    ...
    <div class="outside">
      <div ref="infoBox" class="box_room">
        ...
        <div class="info_box">
          ...
          <el-text v-else class="info_text components-admin-buyrecord-info-box">
            商品销售情况：正常销售中
          </el-text>
        </div>
        <div v-if="record.wupin.tag">
          <el-text class="info_text components-admin-buyrecord-info-box">
            商品标签：{{ record.wupin.tag }}
          </el-text>
        </div>
        <div class="info_box">
          <el-text v-if="record.classId !== 1 && record.wupin && record.wupin.classOf && record.wupin.classOf.id === record.classId" class="info_text components-admin-buyrecord-info-box">
