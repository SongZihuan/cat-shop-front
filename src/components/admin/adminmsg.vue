...
const toUser = () => {
  props.userId && pushTo(router, route, "/admin/user/info", {
    userId: props.userId,
  })
}
...
<template>
  <el-card class="msg_card">
    <div class="msg_text_box">
      ...
      <el-text v-else class="msg_text">
        消息好像消失啦！
      </el-text>
    </div>
    <div v-if="props.user" class="msg_user_box">
      <el-text :class="user_class" @click="toUser">
        留言人：{{  props.user }}
      </el-text>
    </div>
    <div v-if="props.time" class="msg_time_box">
      <el-text class="msg_time">
        留言时间：{{ formatDate(props.time) }}
      </el-text>
    </div>
  </el-card>
</template>
